...
const INFO_TEXTS = {
  ...
  activityLevel: "Choose the level that best describes your typical week — be honest, most people overestimate their activity level.\n\nSedentary: Desk job, little or no exercise. Most of your day is sitting.\n\nLight: Light exercise 1-3 days per week, or a job that involves some walking.\n\nModerate: Moderate exercise 3-5 days per week. You work out regularly but it is not intense.\n\nActive: Hard exercise 6-7 days per week, or a physically demanding job plus regular training.\n\nVery Active: Very hard exercise twice a day, or an extremely physical job like construction or professional athlete level training.\n\nIf in doubt, choose one level lower than you think — it is better to underestimate and adjust than to set an unrealistic target.",
  calorieTarget: "Your daily calorie target is calculated using the Mifflin-St Jeor formula — the most widely validated method for estimating calorie needs.\n\nStep 1: Calculate BMR (Basal Metabolic Rate) — the calories your body needs at complete rest:\nMen: (10 × weight kg) + (6.25 × height cm) − (5 × age) + 5\nWomen: (10 × weight kg) + (6.25 × height cm) − (5 × age) − 161\n\nStep 2: Multiply by your activity level:\nSedentary × 1.2, Light × 1.375, Moderate × 1.55, Active × 1.725, Very Active × 1.9\n\nStep 3: Apply your goal:\nLose Weight: subtract 500 kcal (≈ 0.5kg/week loss)\nMaintain: no change\nGain Muscle: add 300 kcal\n\nThe macro targets (fat, protein, net carbs) are then derived from this calorie total using your chosen diet type framework.",
  magnesiumTarget: "The magnesium target is based on the NIH Recommended Dietary Allowance (RDA): 420mg/day for men and 320mg/day for women aged 31+. Low-carb and Mediterranean diets can deplete magnesium faster as kidneys excrete more during ketosis. Deficiency is very common, estimated at 50-70% of the general population. If your doctor or nutritionist has prescribed a different target, you can override this in your Profile Settings under 'Override Daily Targets'.",
};
...
const MiniChart = ({ data, color, target, label }) => {
  ...
  const avgColor = target ? (avg < target * 0.8 ? "#c9a96e" : avg > target * 1.2 ? "#c9a96e" : "#7ec8a4") : "#aaa";
  return (
...
function UserTracker({ userId, profile, profiles, session }) {
  ...
  const [sodiumSupp, setSodiumSupp] = useState(profile?.defaultSodiumSupp ?? 0);
  ...
  useEffect(() => { if (profile?.defaultSodiumSupp !== undefined) setSodiumSupp(profile.defaultSodiumSupp); }, [profile?.defaultSodiumSupp]);
  ...
  useEffect(() => {
    (async () => {
      ...
      const pid = userId;
      // Only use dbGet if userId looks like a real UUID (not "me" or "wife")
      const isUUID = pid && pid.includes("-") && pid.length > 30;
      const [hist, bl, eo, sl, od, rh, ms2, cs2, mf2, todayEntries] = await Promise.all([
        isUUID ? dbGet(token, pid, "history") : syncFromCloud("history", userId),
        isUUID ? dbGet(token, pid, "burn_log") : syncFromCloud("burn_log", userId),
        isUUID ? dbGet(token, pid, "eaten_override") : syncFromCloud("eaten_override", userId),
        isUUID ? dbGet(token, pid, "supp_log") : syncFromCloud("supp_log", userId),
        isUUID ? dbGet(token, pid, "off_days") : syncFromCloud("off_days", userId),
        isUUID ? dbGet(token, pid, "readings_history") : syncFromCloud("readings_history", userId),
        isUUID ? dbGet(token, pid, "mag_supp") : syncFromCloud("mag_supp", userId),
        isUUID ? dbGet(token, pid, "cal_supp") : syncFromCloud("cal_supp", userId),
        sbGet("keto_shared_my_foods"),
        isUUID ? dbGet(token, pid, "entries_" + todayKey()) : syncFromCloud("entries_" + todayKey(), userId),
      ]);
  ...
  useEffect(() => { if (myFoods.length === 0) return; try { localStorage.setItem("keto_shared_my_foods", JSON.stringify(myFoods)); } catch {} sbSet("keto_shared_my_foods", myFoods); }, [myFoods]);
  ...
  const mineralTotals = {
    sodium: totals.sodium + (sodiumSupp || 0), calcium: totals.calcium,
  ...
  return (
    <div>
      ...
      {/* Minerals */}
      <div style={s.card}>
        <div style={{ ...s.cardLabel, display: "flex", alignItems: "center", gap: 6, marginBottom: 12 }}>Minerals <InfoButton topic="minerals" /></div>
        {[
          { key: "sodium", supp: sodiumSupp, setSupp: setSodiumSupp, step: 250, max: 3000, color: MINERAL_COLORS.sodium },
          { key: "calcium", supp: calSupp, setSupp: setCalSupp, step: profile?.calSuppStep ?? 250, max: 2000, color: MINERAL_COLORS.calcium },
          { key: "magnesium", supp: magSupp, setSupp: setMagSupp, step: 50, max: 400, color: MINERAL_COLORS.magnesium },
          { key: "potassium", supp: 0, setSupp: null, step: 0, max: 0, color: MINERAL_COLORS.potassium },
        ].map(({ key, supp, setSupp, step, max, color }) => {
          const foodVal = mineralTotals[key] - (supp || 0);
          const foodPct = Math.min(Math.max(foodVal, 0) / MINERAL_TARGETS[key], 1);
          const suppPct = supp ? Math.min(supp / MINERAL_TARGETS[key], 1 - foodPct) : 0;
          const over = mineralTotals[key] > MINERAL_TARGETS[key];
          return (
            <div key={key} style={{ marginBottom: 14 }}>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                  {setSupp && (
                    <div style={{ display: "flex", alignItems: "center", gap: 3 }}>
                      {key === "magnesium" && <InfoButton topic="mgSupp" />}
                      <button onClick={() => setSupp(p => Math.max(0, p - step))} style={{ background: "#1e1e1e", border: "1px solid #2a2a2a", borderRadius: 5, width: 20, height: 20, color: "#aaa", fontSize: 13, cursor: "pointer", lineHeight: 1, padding: 0 }}>−</button>
                      <span style={{ fontSize: 10, fontFamily: "monospace", color: supp > 0 ? color : "#444", minWidth: 50, textAlign: "center" }}>{supp > 0 ? `+${supp}mg` : "no supp"}</span>
                      <button onClick={() => setSupp(p => Math.min(max, p + step))} style={{ background: "#1e1e1e", border: "1px solid #2a2a2a", borderRadius: 5, width: 20, height: 20, color: "#aaa", fontSize: 13, cursor: "pointer", lineHeight: 1, padding: 0 }}>+</button>
                    </div>
                  )}
                  <span style={{ fontSize: 11, fontFamily: "monospace", color: over ? "#ff4444" : "#aaa" }}>
                    {Math.round(mineralTotals[key])}<span style={{ color: "#555" }}>/{MINERAL_TARGETS[key]}mg</span>
                  </span>
                </div>
              </div>
              <div style={{ height: 7, borderRadius: 4, background: "#222", overflow: "hidden", position: "relative" }}>
                ...
                {suppPct > 0 && <div style={{ position: "absolute", left: `${foodPct * 100}%`, top: 0, bottom: 0, width: `${suppPct * 100}%`, background: color, opacity: 0.45, borderRadius: 4 }} />}
              </div>
            </div>
          );
        })}
...
function ProfileSetup({ profile, onSave, onCancel, isNew }) {
  ...
  return (
    <div style={{ padding: "16px", paddingBottom: 80 }}>
      ...
      {/* Activity */}
      <div style={s.row}>
        <div style={{ ...s.label, display: "flex", alignItems: "center", gap: 6 }}>Activity Level <InfoButton topic="activityLevel" /></div>
      ...
      <div style={s.row}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, marginBottom: 10 }}>
          <div>
            <div style={{ fontSize: 10, color: "#7ec8e3", marginBottom: 3 }}>Sodium supplement (mg)</div>
            <input type="number" placeholder="e.g. 500"
              value={form.defaultSodiumSupp ?? ""}
              onChange={e => set("defaultSodiumSupp", e.target.value === "" ? undefined : Number(e.target.value))}
              style={{ ...s.input, padding: "8px 10px", fontSize: 12 }} />
          </div>
          <div>
            <div style={{ fontSize: 10, color: "#f4d06f", marginBottom: 3 }}>Calcium supplement (mg)</div>
            <input type="number" placeholder="e.g. 600"
              value={form.defaultCalSupp ?? ""}
              onChange={e => set("defaultCalSupp", e.target.value === "" ? undefined : Number(e.target.value))}
              style={{ ...s.input, padding: "8px 10px", fontSize: 12 }} />
          </div>
          <div>
      ...
      {preview && (
        <div style={{ background: "#161616", borderRadius: 12, padding: "12px 16px", marginBottom: 20, border: "1px solid #2a2a2a" }}>
          <div style={{ fontSize: 11, color: "#888", letterSpacing: 1, textTransform: "uppercase", marginBottom: 10, display: "flex", alignItems: "center", gap: 6 }}>Calculated Targets Preview <InfoButton topic="calorieTarget" /></div>
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Deduplicate by id
        const seen = new Set();
        const allProfiles = [...mapped, ...mappedShared].filter(p => { if (seen.has(p.id)) return false; seen.add(p.id); return true; });
        ...
        setActiveUser(DEFAULT_PROFILES[0].id);
      }
    })();
  }, [session]);
  ...
  if (showHelp) return (
    <div style={{ minHeight: "100vh", background: "#111", color: "#f0ede8", fontFamily: "system-ui, sans-serif", maxWidth: 420, margin: "0 auto", paddingBottom: 40 }}>
      ...
      <div style={{ padding: "16px 16px 0" }}>
        {[
          { title: "Daily Calorie Target", icon: "🔢", content: "Your daily calorie target is calculated using the Mifflin-St Jeor formula. First your BMR (Basal Metabolic Rate) is calculated from your weight, height, age and sex. This is multiplied by your activity level to get your TDEE (Total Daily Energy Expenditure). Your goal adjustment is then applied: Lose Weight subtracts 500 kcal, Maintain keeps it the same, Gain Muscle adds 300 kcal. Your macro targets for fat, protein and net carbs are derived from this total using your chosen diet type framework. You can see these calculated values in Edit Profile under Calculated Targets Preview." },
          { title: "Getting Started", icon: "🚀", content: "Set up your profile first — tap Edit in the top right. Enter your weight, height, age, activity level and goal. The app calculates your daily calorie and macro targets automatically (you can adjust those targets later in your profile settings). Each morning, log the previous day calories burned from your fitness device (Garmin, Apple Watch, etc.). This gives you your deficit — the difference between what you burned and what you ate." },
          { title: "Logging Food", icon: "🍽", content: "Use the Add tab to log food. Under Foods you will find preset foods plus your own saved My Foods list — tap the save icon on any logged item to add it to My Foods. Under AI you can describe a meal in plain language or take a photo of your food and the AI will estimate the nutritional values. Quick Log lets you describe your entire day at once in natural language. Tap any logged food entry to expand and see full nutritional detail including minerals." },
          { title: "History", icon: "📊", content: "The History tab shows your last 7 days. Tap any day to expand and see exactly what you ate. You can log or edit the calories burned for any past day, override the eaten total if you tracked manually, mark a day as an Off Day to exclude it from your stats, and log supplements for past days. The 7-day deficit total at the top shows your cumulative calorie deficit for the week." },
          { title: "Deficit and Calories", icon: "🔥", content: "A positive deficit means you burned more calories than you ate. A negative deficit means you ate more than you burned. Over time, consistent deficits lead to fat loss. 7,700 kcal deficit equals approximately 1kg of fat lost." },
          ...
          { title: "Diet Types", icon: "🥗", content: "Choose your diet framework in Profile Settings. Mediterranean Glycemic Load targets 80g net carbs with balanced fats and protein — ideal for sustainable insulin control. Low Carb targets 50g net carbs. Strict Keto targets 25g net carbs with higher fat ratios. Custom lets you set your own carb target manually. All options calculate your calorie and macro targets based on your personal stats — the diet type just sets the carbohydrate framework." },
          { title: "Minerals", icon: "⚗️", content: "The four key minerals tracked here interact with each other and are commonly depleted on low-carb diets. Sodium and potassium regulate fluid balance and blood pressure. Magnesium supports over 300 enzymatic reactions including sleep and muscle function. Calcium is critical for bone density. Use the +/- controls next to each mineral to log your supplement dose for the day." },
          { title: "Magnesium", icon: "💜", content: "The magnesium target is based on the NIH RDA: 420mg/day for men and 320mg/day for women aged 31+. Low-carb and Mediterranean diets can deplete magnesium faster. You can adjust your supplement dose daily with consideration to your intake from food — if food sources already covered a significant portion, reduce the supplement accordingly." },
          { title: "Glucose and Ketone Readings", icon: "🩸", content: "Access this feature via the Readings tab at the bottom of the screen. Log glucose and ketone readings to track your metabolic health directly. Best times: fasting glucose first thing in the morning, post-meal glucose 1-2 hours after eating, ketones in the afternoon or before bed. Target ranges: fasting glucose 4.0-5.5 mmol/L, post-meal under 7.8 mmol/L, nutritional ketosis 0.5-3.0 mmol/L. You can also log blood pressure readings." },
          { title: "Trends", icon: "📈", content: "Access this via the Trends tab at the bottom of the screen. Charts show your daily values over the selected time range (7, 30, 90 days or all time). The horizontal line is your target. The average shown top-right tells you how you are tracking — if it is consistently below your target you may need to adjust your diet or supplements." },
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          ...
          <div style={{ display: "flex", gap: 6, marginTop: 4 }}>
            ...
            <button onClick={() => window.location.reload()}
              style={{ background: "none", border: "1px solid #7ec8a4", borderRadius: 8, padding: "6px 10px", color: "#7ec8a4", fontSize: 18, cursor: "pointer", fontWeight: 700, lineHeight: 1 }}>
              ↻
            </button>
